...
public:
    ...
    void draw(const Shader &shader, const V3D &color,
              const glm::mat4 &transform, float alpha = 1.0) ;
    void draw(const Shader &shader, const V3D &color, float alpha = 1.0) ;
    void draw(const Shader &shader, float alpha = 1.0) ;

    void drawc(const Shader& shader, const V3D& color,
        const glm::mat4& transform,glm::vec3 p, float alpha = 1.0) ;
    void drawc(const Shader& shader, const V3D& color,glm::vec3 p, float alpha = 1.0) ;
    void drawc(const Shader& shader,glm::vec3 p ,float alpha = 1.0) ;
    void sort_triangles(glm::vec3 p);
