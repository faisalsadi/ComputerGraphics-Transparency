...
void Model::draw(const Shader &shader, const V3D &color,
                 const glm::mat4 &transform, float alpha) {
    shader.use();
    shader.setMat4("model", transform);
    shader.setBool("use_transparency", drawtransparency);
...
void Model::draw(const Shader &shader, const V3D &color, float alpha)  {
    shader.use();
...
void Model::draw(const Shader &shader, float alpha)  {
    shader.use();
    shader.setMat4("model", getTransform());
    for (unsigned int i = 0; i < meshes.size(); i++) {
      //  glm::vec3  k= meshes[i].vertices[meshes[i].indices[0]].position;
        meshes[i].draw(shader, alpha);
    }
}
void Model::sort_triangles(glm::vec3 p)
{
    for (unsigned int i = 0; i < meshes.size(); i++) {
        //  glm::vec3  k= meshes[i].vertices[meshes[i].indices[0]].position;
        meshes[i].sort_tr(p);
